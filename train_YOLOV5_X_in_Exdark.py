...
python train_YOLOV5_X_in_Exdark.py
nohup python train_YOLOV5_X_in_Exdark.py  > train_YOLOV5_X_in_Exdark.out&
3057 GPU1
'''

if not os.path.exists('./log_YOLOV5X_in_Exdark'):
    os.mkdir('./log_YOLOV5X_in_Exdark')

model_save_dir='/export/yuanzhian/lujiajia/paper_code_1/low_light_object_detection/yolov5-pytorch-main/saved_model_YOLOv5_X_in_ExDARK'
...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = '/export/yuanzhian/lujiajia/paper_code_1/low_light_object_detection/yolov5-pytorch-main/model_data/yolov5_x.pth'
    #------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    save_dir            = 'log_YOLOV5X_in_Exdark'
    #------------------------------------------------------------------#
    ...
    input = torch.randn(1, 3, 640, 640)#输入必须是四维的
    # input=input.to(device)#train
    flops, params = profile(model, inputs=(input,))#统计计算量和参数量
    print('***********************************GFLOPs*************************************')
    print('YOLOv5 X的计算量和参数量')
    # logging.info('***********************************GFLOPs*************************************')
    print(flops / 1024 / 1024 /1024) #101.52864456176758
    # logging.info(flops / 1024 / 1024 /1024)
    print('***********************************Para(M)************************************')
    # logging.info('***********************************Para(M)************************************')
    print(params / 1024 / 1024) #83.27330875396729
    # logging.info(params / 1024 / 1024) 
    print('***********************************End****************************************')
    # logging.info('***********************************End****************************************')
    #----------------------#
    #   获得损失函数
    ...
    if local_rank == 0:
        ...
        log_dir         = os.path.join(save_dir, "loss_YOLOv5_X" + str(time_str))
    ...
    if True:
        ...
        if local_rank == 0:
            eval_callback   = EvalCallback(model, input_shape, anchors, anchors_mask, class_names, num_classes, val_lines, log_dir, Cuda, \
                                            eval_flag=eval_flag, period=eval_period,map_out_path='/export/yuanzhian/lujiajia/paper_code_1/low_light_object_detection/yolov5-pytorch-main/log_YOLOV5X_in_Exdark/temp_map_out_YOLOv5_X')
        else:
            eval_callback   = None
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            if local_rank == 0:
                ...
                if (epoch + 1) % save_period == 0 or epoch + 1 == UnFreeze_Epoch:
                    torch.save(save_state_dict, os.path.join(model_save_dir, "ep%03d-loss%.3f-val_loss%.3f.pth" % (epoch + 1, loss / epoch_step, val_loss / epoch_step_val)))
                    
                if len(loss_history.val_loss) <= 1 or (val_loss / epoch_step_val) <= min(loss_history.val_loss):
                    print('Save best model to best_epoch_weights.pth')
                    torch.save(save_state_dict, os.path.join(model_save_dir, "best_epoch_weights.pth"))
                    
                torch.save(save_state_dict, os.path.join(model_save_dir, "last_epoch_weights.pth"))
            if distributed:
                dist.barrier()
